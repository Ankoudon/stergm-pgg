...
log_likelihood <- function(eta, net_list, adj_matrices, list_time_plus) {
  ...
  for (t in 1:time_step) {
    ...
    male_mismatch <- outer(gender_vec == "male", gender_vec == "male", function(x, y) xor(x, y))
    male_hetero_adj <- plus_adj * male_mismatch
    num_male_hetero <- sum(male_hetero_adj) / 2
    
    # Count the number of male homo connections
    male_homo_adj <- plus_adj
    male_homo_adj[gender_vec != "male", ] <- 0
    male_homo_adj[ , gender_vec != "male"] <- 0
    num_male_homo <- sum(male_homo_adj) / 2
    ...
    plus_denominator <- sum(
      ...
      unlist(mclapply(list_time_plus[[t]], function(plus_deno) {
        ...
        # Count the number of male hetero connections
        male_hetero_plus_deno_adj <- plus_deno_adj * male_mismatch
        num_male_hetero_plus_deno <- sum(male_hetero_plus_deno_adj) / 2
        
        # Count the number of male homo connections
        male_homo_plus_deno_adj <- plus_deno_adj
        male_homo_plus_deno_adj[gender_vec != "male", ] <- 0
        male_homo_plus_deno_adj[ , gender_vec != "male"] <- 0
        num_male_homo_plus_deno <- sum(male_homo_plus_deno_adj) / 2
        ...
        exp(
          (sum(plus_deno_adj) / 2) * lambda[1] +
            ...
            num_male_hetero_plus_deno * lambda[6] +
            num_male_homo_plus_deno * lambda[7])
...
# Save the results
write_csv(model_data, "Results/plus_model_gender.csv")
