...
log_likelihood <- function(eta, net_list, adj_matrices, list_time_plus) {
  ...
  for (t in 1:time_step) {
    ...
    num_deg <- as.vector(colSums(plus_adj))
    ...
    plus_denominator <- sum(
      ...
      unlist(mclapply(list_time_plus[[t]], function(plus_deno) {
        ...
        num_plus_deno_deg <- as.vector(colSums(plus_deno_adj))
        num_plus_deno_2_star <- sum(choose(num_plus_deno_deg, 2))
...
# Save the results
write_csv(model_data, "Results/plus_model_2_star.csv")
