# This script fits the public goods game data with male homo and male hetero terms.
  ...
  for (t in 1:time_step) {
    ...
    minus_numerator <- exp(
      (sum(minus_adj) / 2) * lambda[1] +
        ...
        num_male_hetero * lambda[6] +
        num_minus_male_homo * lambda[7]
        )
      ...
      unlist(mclapply(list_time_minus[[t]], function(minus_deno) {
        ...
        exp(
          (sum(minus_deno_adj) / 2) * lambda[1] +
            ...
            num_male_hetero_minus_deno * lambda[6] +
            num_male_homo_minus_deno * lambda[7] 
            )
...
result <- optim(par = c(0, 0, 0, 0, 0, 0, 0),
...
# Save the results
write_csv(model_data, "Results/minus_model_gender.csv")
