  ...
  for (t in 1:time_step) {
    ...
    minus_numerator <- exp(
      (sum(minus_adj) / 2) * lambda[1] +
        num_minus_coop_homo * lambda[2] +
        num_minus_def_homo * lambda[3] +
        num_minus_score * lambda[4])
      ...
      unlist(mclapply(list_time_minus[[t]], function(minus_deno) {
        ...
        exp(
          (sum(minus_deno_adj) / 2) * lambda[1] +
            num_coop_homo_minus_deno * lambda[2] +
            num_def_homo_minus_deno * lambda[3] +
            num_minus_deno_score * lambda[4])
...
# Save the results
write_csv(model_data, "Results/minus_model_no_triangle.csv")
