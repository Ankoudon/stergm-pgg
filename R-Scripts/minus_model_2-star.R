# This script fits the public goods game data with the 2-star term.
...
log_likelihood <- function(eta, net_list, adj_matrices, list_time_minus) {
  ...
  for (t in 1:time_step) {
    ...
    # Count the number of male hetero connections
    male_mismatch <- outer(
      gender_vec == "male", gender_vec == "male", function(x, y) xor(x, y))
    male_hetero_adj <- minus_adj * male_mismatch
    num_male_hetero <- sum(male_hetero_adj) / 2
    
    # Count the number of male homo connections
    minus_male_homo_adj <- minus_adj
    minus_male_homo_adj[gender_vec != "male", ] <- 0
    minus_male_homo_adj[ , gender_vec != "male"] <- 0
    num_minus_male_homo <- sum(minus_male_homo_adj) / 2
    ...
    # Count the number of 2-stars 
    num_deg <- as.vector(colSums(minus_adj))
    num_2_star <- sum(choose(num_deg, 2))
    ...
    minus_denominator <- sum(
      ...
      unlist(mclapply(list_time_minus[[t]], function(minus_deno) {
        ...
        # Count the number of male hetero connections
        male_hetero_minus_deno_adj <- minus_deno_adj * male_mismatch
        num_male_hetero_minus_deno <- sum(male_hetero_minus_deno_adj) / 2
        
        # Count the number of male homo connections
        male_homo_minus_deno_adj <- minus_deno_adj
        male_homo_minus_deno_adj[gender_vec != "male", ] <- 0
        male_homo_minus_deno_adj[ , gender_vec != "male"] <- 0
        num_male_homo_minus_deno <- sum(male_homo_minus_deno_adj) / 2
        ...
        # Count the number of 2-star
        num_minus_deno_deg <- as.vector(colSums(minus_deno_adj))
        num_minus_deno_2_star <- sum(choose(num_minus_deno_deg, 2))
        ...
        exp(
          (sum(minus_deno_adj) / 2) * lambda[1] +
            ...
            num_minus_deno_2_star * lambda[6] 
        )
...
result <- optim(par = c(0, 0, 0, 0, 0, 0),
...
# Save the results
write_csv(model_data, "Results/minus_model_2-star.csv")
